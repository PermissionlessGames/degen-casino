...
contract DevPCPricingTest is Test {
    ...
    function testGetAllCurrencyPrices() public view {
        (bytes[] memory currencies, uint256[] memory prices) = devPCPricing.getAllCurrencyPrices();
    ...
    function testGetAdjustmentFactors() public view {
        (uint256 numerator, uint256 denominator) = devPCPricing.getAdjustmentFactor();
    ...
    function testCurrencyExists() public view {
        ...
        assertFalse(devPCPricing.currencyExists("NONEXISTENT"), "Non-existent currency should return false");
    }

    function testRemoveCurrency() public {
        assertTrue(devPCPricing.currencyExists(USDT), "USDT should exist before removal");
        devPCPricing.removeCurrency(USDT);
        assertFalse(devPCPricing.currencyExists(USDT), "USDT should not exist after removal");
    }
    ...
    function testCurrencyExistBothFalseAndTrue() public view {
        assertTrue(devPCPricing.currencyExists(ETH), "ETH should exist");
        assertFalse(devPCPricing.currencyExists("NON_EXISTENT"), "NON_EXISTENT should not exist");
    }
    ...
    function testBatchProcessing() public {
        ...
        (uint256 lastIndex, uint256 total) = devPCPricing.getBatchProcessingState();
        ...
        assertEq(lastIndex, 0, "Index should reset after processing all currencies");

        // Verify all prices were adjusted
        assertEq(devPCPricing.getCurrencyPrice(USDT), 95, "USDT price should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(GOLD), 475, "GOLD price should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(USDC), 95, "USDC price should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(DAI), 95, "DAI price should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(BTC), 28500, "BTC price should be adjusted");
    }

    function testBatchProcessingWithSmallBatches() public {
        // Add more currencies
        for (uint256 i = 0; i < 10; i++) {
            bytes memory currency = bytes(string.concat("TOKEN", vm.toString(i)));
        ...
        for (uint256 i = 0; i < 10; i++) {
            bytes memory currency = bytes(string.concat("TOKEN", vm.toString(i)));
            assertEq(devPCPricing.getCurrencyPrice(currency), 95, "Price should be adjusted");
        }
        assertEq(devPCPricing.getCurrencyPrice(USDT), 95, "USDT price should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(GOLD), 475, "GOLD price should be adjusted");
    }
    ...
    function testSetBatchSize() public {
        ...
        assertEq(devPCPricing.getBatchSize(), 5, "Batch size should be set to 5");
    }
    ...
    function testAutomaticBatchProcessing() public {
        ...
        assertEq(total, 12, "Should have 12 total currencies (10 + USDT + GOLD)");

        // Verify first batch prices were adjusted
        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN0")), 95, "TOKEN0 should be adjusted");
        // Second batch should process next 3 tokens
        devPCPricing.useAnchorCurrency(false);
        (lastIndex,) = devPCPricing.getBatchProcessingState();
        assertEq(lastIndex, 6, "Should process next 3 tokens");

        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN1")), 95, "TOKEN1 should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN2")), 95, "TOKEN2 should be adjusted");

        // Verify second batch prices were adjusted
        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN3")), 95, "TOKEN3 should be adjusted");

        devPCPricing.useAnchorCurrency(false);
        (lastIndex,) = devPCPricing.getBatchProcessingState();
        assertEq(lastIndex, 9, "Should process next 3 tokens");

        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN4")), 95, "TOKEN4 should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN5")), 95, "TOKEN5 should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(bytes("TOKEN6")), 95, "TOKEN6 should be adjusted");
        ...
        assertEq(devPCPricing.getCurrencyPrice(USDT), 91, "USDT should be adjusted");
        assertEq(devPCPricing.getCurrencyPrice(GOLD), 452, "GOLD should be adjusted");
    }
}
