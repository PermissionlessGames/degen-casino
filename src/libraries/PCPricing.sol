...
library PCPricing {
    ...
    function setCurrencyPrice(PricingData storage self, bytes memory currency, uint256 price) internal {
        ...
        require(keccak256(currency) != keccak256(bytes("")), "Currency cannot be empty");
        require(keccak256(self.anchorCurrency) != keccak256(currency), "Cannot set price for anchor currency");
    ...
    function adjustCurrencyPrice(PricingData storage self, bytes memory currency, bool increase) internal {
        require(keccak256(self.anchorCurrency) != keccak256(currency), "Anchor currency cannot be adjusted");
        require(self.currencyPrice[currency] > 0, "Currency price not set");

        uint256 adjustmentAmount =
            (self.currencyPrice[currency] * self.adjustmentNumerator) / self.adjustmentDenominator;
        ...
        if (increase) {
            ...
            self.currencyPrice[currency] =
                self.currencyPrice[currency] > adjustmentAmount ? self.currencyPrice[currency] - adjustmentAmount : 1;
        }

        emit CurrencyPriceAdjusted(currency, self.currencyPrice[currency], increase);
    }
    ...
    function removeCurrency(PricingData storage self, bytes memory currency) internal {
        ...
        require(keccak256(currency) != keccak256(bytes("")), "Currency cannot be empty");
        require(keccak256(self.anchorCurrency) != keccak256(currency), "Cannot remove anchor currency");
        uint256 index = self.currencyIndex[currency];
        if (index < self.trackedCurrencies.length - 1) {
            self.trackedCurrencies[index] = self.trackedCurrencies[self.trackedCurrencies.length - 1];
