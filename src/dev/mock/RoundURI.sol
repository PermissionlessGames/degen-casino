...
contract PositionMetadata {
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        metadataBytes(positionTokenID, position, pool)
                    )
                )
            );
    }
    ...
    function generateSVGForeground(
        ...
        string memory tokenAmountOrIdLabel = pool.tokenType == 721
            ? "Token ID"
            : "Amount staked";
        string memory amountOrTokenIdString = (position.amountOrTokenID)
            .toString();

        // Timestamp string manipulations
        string memory stakeTimestampStr = formatDateTime(
            DateTime.parseTimestamp(position.stakeTimestamp)
        );
        string memory unlockTimestampStr = formatDateTime(
            DateTime.parseTimestamp(
                position.stakeTimestamp + pool.lockupSeconds
            )
        );
        ...
        string memory tokenAddressString = Strings.toHexString(
            uint256(uint160(pool.tokenAddress)),
            20
        );
    ...
    function generateTokenTypeElement(
        ...
        if (
            keccak256(abi.encodePacked(tokenTypeString)) ==
    ...
    function getAddressSlice(
        address tokenAddress
    ) public pure returns (string memory) {
        // Convert address to a hex string
        string memory hexAddress = Strings.toHexString(
