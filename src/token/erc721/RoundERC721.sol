    ...
    IERC721Errors
{
    ...
    mapping(uint256 roundId => mapping(uint256 tokenId => address))
        private _owners;

    mapping(uint256 roundId => mapping(address owner => uint256))
        private _balances;

    mapping(uint256 roundId => mapping(uint256 tokenId => address))
        private _tokenApprovals;

    mapping(uint256 roundId => mapping(address owner => mapping(address operator => bool)))
    ...
    function _ownerOf(uint256 tokenId) internal view virtual returns (address) {
        return _owners[roundId][tokenId];
    }
    ...
    function _increaseBalance(address account, uint128 value) internal virtual {
        unchecked {
            _balances[roundId][account] += value;
        }
    }
